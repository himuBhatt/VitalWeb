...
export function MedicineReminderContent() {
  ...
  useEffect(() => {
    ...
    const fetchPatients = async () => {
      try {
        ...
        if (storedUid) {
          setSelectedPatientId(storedUid);
          const found = list.find(p => p.id === storedUid);
          if (found) setForm(prev => ({ ...prev, user: found.name || storedUid }));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const storedUid = typeof window !== "undefined" ? localStorage.getItem("patientUid") : null;
      const pid = selectedPatientId || storedUid || null;
